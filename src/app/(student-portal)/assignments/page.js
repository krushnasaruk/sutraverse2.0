...
import { IconAssignment, IconUser, IconHat, IconDownload, IconStar, IconSearch, IconLock, IconFlag } from '@/frontend/components/ui/Icons';
...
export default function AssignmentsPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        if (!cancelled) setAssignments(verified);
      } catch (error) {
  ...
  const filtered = useMemo(() => {
    ...
    // Filter by branch and year (case-insensitive, defaults/All match everything)
    r = r.filter(a => {
      const bMatch = !a.branch || a.branch === 'All' || a.branch.toLowerCase() === branch.toLowerCase();
      const yMatch = !a.year || a.year === 'All' || a.year.toLowerCase() === year.toLowerCase();
    ...
    return r;
  }, [assignments, activeTab, search, branch, year]);
