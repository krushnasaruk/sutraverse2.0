...
export const handlePost_generatestudyguide = async (request) => {
    ...
    try {
        ...
        // 1.5 Check dynamic local cache next
        const cachePath = path.join(process.cwd(), 'public', 'data', 'ai_generated_guides.json');
        const cacheKey = `${year}_${branch}_${subject}`.toLowerCase().replace(/[^a-z0-9]/g, '_');
        if (fs.existsSync(cachePath)) {
            try {
                const cacheData = JSON.parse(fs.readFileSync(cachePath, 'utf-8'));
                if (cacheData[cacheKey]) {
                    console.log(`[API] Serving cached dynamic study guide for: ${cacheKey}`);
                    return NextResponse.json(cacheData[cacheKey]);
                }
            } catch (cacheErr) {
                console.error('Error reading/parsing dynamic cache:', cacheErr);
            }
        }
        ...
        const model = genAI.getGenerativeModel({
            ...
            generationConfig: {
                ...
                responseSchema: {
                    ...
                    required: ["summaries", "questions", "flashcards"]
                }
            }
        });
        ...
        try {
            ...
            cacheData[cacheKey] = parsedJson;
            fs.writeFileSync(cachePath, JSON.stringify(cacheData, null, 2), 'utf-8');
            console.log(`[API] Cached generated study guide for: ${cacheKey}`);
        } catch (saveErr) {
            console.error('Failed to save to dynamic study guide cache:', saveErr);
        }
...
export const handlePost_notificationsgenerateandsend = async (request) => {
  try {
    ...
    const { contentType, contentTitle, contentDetails, contentId } = await request.json();
